...
def test_get_teachers(setup_database):
    ...
    # 使用获取的token访问教师列表
    response = client.get(f"/api/v1/teachers?token={token}")
...
def test_get_teacher_by_id(setup_database):
    ...
    # 使用获取的token访问教师详情
    response = client.get(f"/api/v1/teachers/3001?token={token}")
    assert response.status_code == 200
    data = response.json()
...
def test_get_nonexistent_teacher(setup_database):
    ...
    # 使用获取的token访问不存在的教师
    response = client.get(f"/api/v1/teachers/9999?token={token}")
